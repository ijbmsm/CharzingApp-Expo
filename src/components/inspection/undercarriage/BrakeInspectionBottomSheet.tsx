/**
 * BrakeInspectionBottomSheet - 브레이크 검사 (v2)
 * 5개 항목 (brakeOil, padF, padR, discF, discR)
...
interface BrakeInspectionBottomSheetProps {
  visible: boolean;
  onClose: () => void;
  onSave: (data: Record<BrakeKey, BaseInspectionItem>) => void;
  initialData?: Record<BrakeKey, BaseInspectionItem>;
}

const BrakeInspectionBottomSheet: React.FC<BrakeInspectionBottomSheetProps> = ({
  visible,
  onClose,
  onSave,
  initialData = {},
  ...
  const [brakeData, setBrakeData] = useState<Record<string, BaseInspectionItem>>({});

  useEffect(() => {
    if (visible) {
      const dataMap: Record<string, BaseInspectionItem> = {};
      BRAKE_KEYS.forEach((key) => {
        dataMap[key] = initialData[key] || {};
      });
      setBrakeData(dataMap);
    }
  }, [visible, initialData]);

  const handleStatusChange = (key: BrakeKey, status: 'good' | 'problem' | undefined) => {
    setBrakeData((prev) => ({
      ...prev,
      [key]: { ...prev[key], status },
    }));
  ...
  const handleDescriptionChange = (key: BrakeKey, description: string) => {
    setBrakeData((prev) => ({
      ...prev,
  ...
  const handleImagesAdded = (key: BrakeKey, newUris: string[]) => {
    setBrakeData((prev) => ({
      ...prev,
      [key]: {
        ...prev[key],
  ...
  const handleImageRemoved = (key: BrakeKey, index: number) => {
    setBrakeData((prev) => ({
      ...prev,
      [key]: {
        ...prev[key],
  ...
  const handleSave = () => {
    const result: Record<BrakeKey, BaseInspectionItem> = {} as Record<BrakeKey, BaseInspectionItem>;
    BRAKE_KEYS.forEach((key) => {
...
export default BrakeInspectionBottomSheet;
